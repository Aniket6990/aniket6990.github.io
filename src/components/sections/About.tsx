...
const HEADER = "Building the infrastructure behind modern Web3 applications.";
...
const PROFILE_SOCIALS = ["GitHub", "LinkedIn", "Twitter"];

export default function About() {
  const filteredSocials = socials.filter((s) =>
    PROFILE_SOCIALS.includes(s.label),
  );
  ...
  return (
    <section id="about" className="px-8 lg:px-12 py-12">
      ...
      >
        ...
        {/* Identity text */}
        <div>
          <h1 className="text-2xl font-bold text-dark tracking-tight mb-0.5">
            Aniket Singh
          </h1>
      ...
      >
        {SUMMARY.split("\n\n").map((para, i) => (
          <p
            key={i}
            className="text-dark/60 text-sm lg:text-base leading-relaxed"
          >
            {para}
          </p>
        ))}
