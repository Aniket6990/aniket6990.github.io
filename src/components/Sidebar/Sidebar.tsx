...
export default function Sidebar({ active, onNav, collapsed }: SidebarProps) {
  return (
    <aside className={`flex flex-col h-full bg-dark overflow-hidden ${collapsed ? 'items-center py-6 px-3' : 'px-6 py-8'}`}>

      {/* ── Profile ── */}
      <div className={`shrink-0 ${collapsed ? 'mb-6 mt-2 flex flex-col items-center' : 'mb-8 flex items-center gap-3'}`}>
        {/* Avatar */}
      ...
      {/* ── Divider ── */}
      <div className={`w-full h-px bg-light/10 mb-6 shrink-0 ${collapsed ? 'w-8' : ''}`} />
      ...
      {/* ── Divider ── */}
      <div className={`h-px bg-light/10 mt-6 mb-5 shrink-0 ${collapsed ? 'w-8' : 'w-full'}`} />

      {/* ── Socials ── */}
      <div className={`shrink-0 ${collapsed ? 'flex flex-col items-center gap-2' : ''}`}>
        <AnimatePresence initial={false}>
          {!collapsed && (
            <motion.p
              key="socials-label"
        ...
        <div className={`flex gap-2 ${collapsed ? 'flex-col items-center' : 'flex-wrap'}`}>
          {socials.map((s) => (
            <a
              key={s.label}
              href={s.href}
            ...
            >
              <span className="w-3.5 h-3.5" dangerouslySetInnerHTML={{ __html: s.icon }} />
            </a>
          ))}
        </div>
      </div>
